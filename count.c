...
int main(int argc,char* argv[]){
	...
	if(argc!=4){
		printf("The argument number should be four, like \"count\
						in_path search_pattern out_path\n\"");
		return -1;
	}
	...
	if((fout=fopen(path_out,"w"))==0){
		printf("No output file named %s, and error happened when trying to create it.\n",\
						path_out);
	}
