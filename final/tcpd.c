...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        if (ready > 0)
        {
            ...
            struct connected_sock *TmpConnectedSocket;
            ...
            {
                if (FD_ISSET(TmpConnectedSocket->SocketFD, &sock_fd))
                {
                    g_info("Received Ack");
                    recv_ack(TmpConnectedSocket);
                }
            }
...
//receive from the troll or client
    ...
    //send ACK
    ssize_t AckRet = sendto(NowBindedSocket->SocketFD, (void *)NowTrollAck, sizeof(*NowTrollAck), MSG_WAITALL, (struct sockaddr *)&troll_addr, sizeof(troll_addr));
    if (AckRet < 0)
    {
...
ERRCODE handle_send_all_back()
{
    ...
    while (g_hash_table_iter_next(&ConnectedSocketIter, (void *)&ConnectedSocketSocketFD, (void *)&TmpConnectedSocket) != FALSE)
    {
        ...
        while (l != NULL)
        {
            ...
            ssize_t SendRet = sendto(TmpConnectedSocket->SocketFD, (void *)NowTrollMessage, sizeof(*NowTrollMessage), MSG_WAITALL, (struct sockaddr *)&troll_addr, sizeof(troll_addr));
            if (SendRet < 0)
            {
...
ERRCODE tcpd_BIND(struct sockaddr_in FrontAddr, socklen_t FrontAddrLength)
{
    ...
    struct binded_sock *NowBindedSocket = (struct binded_sock *)malloc(sizeof(struct binded_sock));
    NowBindedSocket->SocketFD = sockfd;
    NowBindedSocket->Accepting = FALSE;
    g_array_append_val(binded_sockets, NowBindedSocket);
...
void re_register_fd()
{
    ...
    while (g_hash_table_iter_next(&ConnectedSocketIter, (void *)&ConnectedSocketSocketFD, (void *)&TmpConnectedSocket))
    {
        FD_SET(TmpConnectedSocket->SocketFD, &sock_fd);
        max_socket = TmpConnectedSocket->SocketFD > max_socket ? TmpConnectedSocket->SocketFD : max_socket;
